...
export default function CategoryLayout({ children, params }: CategoryLayoutProps) {
  ...
  return (
    <div className="flex flex-col w-full h-screen fixed">
      {isLargeScreen ? (
        <header className="w-full h-[67px] flex items-center">
          <div className="flex">
            <h1 onClick={toggleModal} className="cursor-pointer flex items-center">
              <Text variant="header-20" className="ml-5">
                {currentCategory === "0"
                  ? "에그팝"
                  : categories.find((cat) => cat.main_category_id === Number(currentCategory))?.main_category_name}
              </Text>
              <span className="ml-1">
            ...
            {isModalOpen && (
              <div className="absolute top-16 left-5 bg-white rounded-lg shadow-lg z-50 border border-solid border-gray-100">
                <div className="w-[288px] grid grid-cols-2 rounded-lg overflow-hidden">
                  {/* 첫 번째 칼럼 */}
                  <div className="flex flex-col">
                    <Link href="/category/0">
                      <div className="h-[60px] flex items-center justify-center border-solid border-b border-r border-gray-100">
                        <Text variant="body_medium-14" className="text-gray-900">
                          에그팝
                        </Text>
                      </div>
                    </Link>
                    ...
                    <Link href={`/category/${categories[1]?.main_category_id}`}>
                      <div className="h-[60px] flex items-center justify-center border-solid border-r border-gray-100">
                        <Text variant="body_medium-14" className="text-gray-900">
                  ...
                  <div className="flex flex-col">
                    <Link href={`/category/${categories[0]?.main_category_id}`}>
                      <div className="h-[60px] flex items-center justify-center border-solid border-b border-gray-100">
                        <Text variant="body_medium-14" className="text-gray-900">
                    ...
                    <div className="h-[60px] flex items-center justify-center">
                      <Text variant="body_medium-14" className="text-gray-300">
                        곧 생겨요!
                      </Text>
                    </div>
                  </div>
