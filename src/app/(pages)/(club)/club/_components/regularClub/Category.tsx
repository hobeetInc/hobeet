...
const Category = ({ formData, setFormData }: EggClubProps) => {
  ...
  return (
    <div>
      ...
      <div
        className={`flex flex-col gap-2 ${isLargeScreen ? "h-[570px]" : "h-[562px]"} overflow-y-auto scrollbar-hide`}
      >
              ...
              <div className="grid-cols-3 grid gap-2 p-3 pt-0">
                {getSubCategory(main.main_category_id).map((sub) => (
                  <div
                    ...
                    className={`min-w-[106px] h-[43px] px-2 py-3 rounded-lg border border-solid border-[#d9d9d9] justify-center items-center inline-flex ${
                      formData.sub_category_id === sub.sub_category_id && "border-primary-900 border-2 bg-primary-900"
                    }`}
                  >
                    <Text
                      variant="subtitle-14"
